...
public class FilesFragment extends Fragment implements Observer {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        Button playButton = view.findViewById(R.id.play_button); // 假设在布局中添加了 play_button
        playButton.setOnClickListener(v -> {
            // 导航到 ReplayFragment，并传递轨迹文件路径作为参数
            //NavDirections action = FilesFragmentDirections.actionFilesFragmentToReplayFragment()
            //        .setTrajectoryFilePath("/path/to/trajectory.proto"); // 设置轨迹文件路径
            //Navigation.findNavController(view).navigate(action);

            NavDirections action = FilesFragmentDirections.actionFilesFragmentToReplayFragment("/path/to/trajectory.proto");
            Navigation.findNavController(view).navigate(action);
        });
